...
object ThinClientCacheEx {
  ...
  def main(args: Array[String]): Unit = withIgniteThinClient("127.0.0.1", 10802) { implicit client =>
    // only inMemory cache
    {
      ...
      val cache: ClientCache[Int, String] = client.getOrCreateCache[Int, String](conf)
